import type { Config } from "tailwindcss";

export default {
  content: [
    "./src/pages/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/components/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/app/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  safelist: [
    { 
      pattern: /(bg|text|border|hover:bg)-sait(Red|Blue|Purple|Gray|LightRed|LightBlue|LightPurple|DarkRed|DarkBlue|DarkPurple|LighterRed|LighterBlue|LighterPurple)/ 
    },
    // all gradient classes
    'bg-gradient-to-r',
    'from-slate-700',
    'via-saitDarkPurple',
    'to-saitBlue',
    // image-related classes
    'object-cover',
    'rounded-full',
    'border-4',
    'border-white',
    // dynamic classes
    { pattern: /^text-(sm|base|lg|xl)/ },
    { pattern: /^font-(normal|semibold|bold)/ },
    { pattern: /^rounded-(none|sm|md|lg|full)/ },
    // 'bg-green-600',
    // 'bg-green-700',
    // 'bg-green-800',
    // 'bg-green-900',
    // 'bg-slate-300',
    // 'bg-slate-600',
    // 'bg-slate-700',
    // 'bg-slate-800',
    // 'bg-slate-900',
    // 'bg-saitRed',
    // 'bg-saitDarkRed',
    // 'bg-saitLighterBlueOg',
    // 'bg-saitLighterBlue',
    // 'bg-saitLightBlue',
    // 'bg-saitBlue',
    // 'bg-saitBlueOg',
    // 'bg-saitPurple',
    // 'bg-saitDarkPurple',
    // 'bg-saitLighterPurple',
    // 'bg-saitLightPurple',
    // 'bg-saitGray',
    // 'ng-saitBlack',
    // 'bg-saitWhite',
    // 'hover:bg-green-500',
    // 'hover:bg-slate-800',
    // 'text-green-200',
    // 'text-green-700',
    // 'hover:text-green-200',
],
  theme: {
    extend: {
      colors: {
        background: "var(--background)",
        foreground: "var(--foreground)",
        saitBlack: "#06222b",
        saitWhite: "#efefef",
        saitRed: "#ca3929",
        saitLightRed: "#ff8b7e",
        saitDarkRed: "#932728",
        saitLighterBlueOg: "#449dde", 
        saitLighterBlue: "#449dde",        // original : out of contrast with white
        saitLightBlue: "#2b64ae",
        saitBlue: "#005795",
        saitBlueOg: "#2b64ae",
        saitPurple: "#5c2876",
        saitDarkPurple: "#502267",
        saitLighterPurple: "#722a7a",
        saitLightPurple: "#8d4990",
        saitGray: "#60605f",
        // saitGray: "#666666",
      },
      backgroundImage: {
        "blue-gradient": `linear-gradient(to top, #2b64ae, #449dde)`,
        "purple-gradient": `linear-gradient(to top, #722a7a, #8d4990)`,
        "red-gradient": `linear-gradient(to top, #ff8b7e, #ca3929)`,
        "side-red-gradient": `linear-gradient(to right, #e43f2e, #932728)`,
        // "side-red-gradient": `linear-gradient(to right, #ca3929, #932728)`,
      },
      screens: {
        '2xs': '320px',
        'xs': '375px',
        'xl': '1440px',
        '2xl': '1536px',
        '955': '955px',
      }
    },
  },
  plugins: [],
} satisfies Config;
